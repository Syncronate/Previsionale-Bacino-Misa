...
class HydroLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.view(out.size(0), out.size(0), self.output_size) # corrected view to out.size(0), self.output_window, self.output_size
        out = out.view(out.size(0), self.output_window, self.output_size) # corrected view to out.size(0), self.output_window, self.output_size
...
def load_model(model_path, input_size, hidden_size, output_size, output_window, num_layers=2):
    ...
    try:
        ...
        st.error(f"Dettagli errore: {e}") # added to show full error message
...
target_columns = hydro_features  # Tutti gli idrometri sono target
...
if use_demo_files or (data_file and model_file and scaler_features_file and scaler_targets_file):
    try:
        ...
        st.sidebar.write(f"Output size check: {output_size * OUTPUT_WINDOW}") # debug line

        if use_demo_files:
            model, device = load_model(MODEL_PATH, input_size, hidden_size, output_size, OUTPUT_WINDOW)
            scaler_features, scaler_targets = load_scalers(SCALER_FEATURES_PATH, SCALER_TARGETS_PATH)
        else:
            # Per i file caricati dall'utente
            model_bytes = io.BytesIO(model_file.read())
            model, device = load_model(model_bytes, input_size, hidden_size, output_size, OUTPUT_WINDOW)
